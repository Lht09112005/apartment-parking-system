...
// POST /api/parking/monthly/register (Hàm dành cho Cư dân gửi yêu cầu)
const registerMonthlyParking = async (req, res) => {
  const { plate_number, months, start_date } = req.body;

  if (!plate_number || !months) {
    return res.status(400).json({ message: "Vui lòng nhập biển số xe và số tháng muốn đăng ký" });
  }

  try {
    // 1. Kiểm tra xe có tồn tại hợp lệ và thuộc cư dân nào không
    const [vehicles] = await db.query(
      `SELECT v.plate_number, v.resident_id, r.name as resident_name, r.apartment_number 
       FROM vehicles v
       JOIN residents r ON v.resident_id = r.resident_id
       WHERE v.plate_number = ? AND v.status = 'active'`,
      [plate_number]
    );

    if (vehicles.length === 0) {
      return res.status(404).json({ message: "Xe không tồn tại, chưa được duyệt hoặc không thuộc quyền sở hữu của cư dân" });
    }

    const vehicle = vehicles[0];

    // 2. Kiểm tra xem xe này có đơn đăng ký vé tháng nào đang chờ duyệt hoặc vẫn đang còn hạn sử dụng không
    const [existingTickets] = await db.query(
      `SELECT status, end_date FROM monthly_parking 
       WHERE plate_number = ? AND (status = 'pending' OR (status = 'active' AND end_date >= CURDATE()))`,
      [plate_number]
    );
    ...
    if (existingTickets.length > 0) {
      ...
      if (ticket.status === 'pending') {
        return res.status(400).json({ message: "Xe này đã có một yêu cầu đăng ký vé tháng đang chờ duyệt" });
      } else {
        const expiryDate = new Date(ticket.end_date).toLocaleDateString("vi-VN");
        return res.status(400).json({ message: `Xe này đã có vé tháng đang hoạt động (Hạn dùng đến: ${expiryDate})` });
      }
    }

    // 3. Tính toán ngày bắt đầu và ngày kết thúc dự kiến dựa trên số tháng đăng ký
    const startDate = start_date ? new Date(start_date) : new Date();
    const endDate = new Date(startDate);
    endDate.setMonth(endDate.getMonth() + parseInt(months));

    // 4. Lưu yêu cầu vào bảng với trạng thái 'pending'
    const [result] = await db.query(
      `INSERT INTO monthly_parking (plate_number, start_date, end_date, status) VALUES (?, ?, ?, 'pending')`,
      [plate_number, startDate, endDate]
    );

    // 5. GỬI THÔNG BÁO realtime & lưu database cho Super Admin & Admin (Role 1 & 2)
    await NotificationService.notifyRole(
      [1, 2],
      "Yêu cầu duyệt vé tháng",
      `Cư dân ${vehicle.resident_name} (Căn hộ ${vehicle.apartment_number}) đã gửi yêu cầu đăng ký vé tháng cho xe biển số ${plate_number}.`,
      "MONTHLY_APPROVAL_REQUEST"
    );

    res.status(201).json({ 
      message: "Gửi yêu cầu đăng ký vé tháng thành công, vui lòng chờ Ban quản lý phê duyệt",
      monthly_id: result.insertId
    });
  } catch (err) {
    console.error("Lỗi đăng ký vé tháng:", err);
    res.status(500).json({ message: "Lỗi server khi xử lý đăng ký vé tháng" });
  }
};
